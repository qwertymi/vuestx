<template>
  ...
  <header class="header">
    <div class="inner">
      <a href="http://www.naver.com" v-on:click.prevent class="logo">stx건설</a>
      <nav class="gnb">

        <ul class="depth1">

          <li v-for="(item, index) in gnbData" v-bind:key="index">
            <a v-bind:href="item.mainlink">{{item.maintxt}}</a>
            <ul class="depth2">
              <li v-for="(subitem, subindex) in item.subdata" v-bind:key="subindex">
